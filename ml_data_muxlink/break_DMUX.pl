...
my %dec_key_bits=();
my %key_bits=();
open (FH,'<', "./${file_name}/cell.txt") or die $!;
...
open (FH,'<', "./${file_name}/links_test_${h}__pred.txt") or die $!;
while (<FH>){
my $line= $_;
my @columns=split(/\s+/,$line);
push @pair1, $columns[0];
...
close(FH);
open (FH,'<', "./${file_name}/link_test_n_${h}__pred.txt") or die $!;
while (<FH>){
my $line= $_;
my @columns=split(/\s+/,$line);
push @pair2, $columns[0];
 $connect_from_all{$columns[0]." ".$columns[1]}= $columns[2];
...
open (FH,'<', "check_${circuit_name}.txt") or die $!;
while (<FH>){
my $line= $_;
if ($line=~/#key=(\d+)\s*/){
$correct_key=$1;
...
my $lik1_two= $connect_from_all{$cells{$path1}." ".$cells{$output_two}};
my $dif2=abs($lik0_two - $lik1_two);
#print "like is $lik0 $lik1 $lik0_two $lik1_two\n";
#print "likelihood of $lik0_two and likelihood of path 1 is $lik1_two\n
#print "Dif is $dif\n";
#print "Dif2 is $dif2\n";
#print "DIF1 is $dif and difference two is $dif2\n";
if ($dif>=$th || $dif2>=$th){
#print "one of the differences is greater than threshold\n";
if ($dif>$dif2){
#print "Dif1 is greater\n";
if ($lik0>$lik1)
{
#print "For which path 0 is greater\n";
#print "keyinput$key is 0\n";
$dec_key_bits{$key}="0";}
else{
#print "For which path 1 is greater\n";
#print "keyinput$key is 1\n";
$dec_key_bits{$key}="1";}}#end if difference 1 is greater than dif2
elsif ($dif2>$dif){
#print "Dif2 is greater\n";
if ($lik0_two>$lik1_two)
{
#print "Path1 is greater so keyinput$key is 1\n";
#print "Path 1 is $path0 to $output_two\n";
$dec_key_bits{$key}="1";}
else{
#print "Path0 is greater so keyinput$key is 0\n";
#print "Path0 is $path1 to $output_two\n";
$dec_key_bits{$key}="0";}}#end if difference 2 is greater than dif2
else { #they are equal
if ($lik0> $lik1){if ($lik0>$lik0_two){
$dec_key_bits{$key}="0";}
