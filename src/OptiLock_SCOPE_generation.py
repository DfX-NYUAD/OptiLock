...
def write_sbatch(file, job_name, memory, walltime, output, job_num, root_dir, circuit_name, h_hop, train_mark, key_size, start_num, total_num, iter_num, init_temp, bin_num, output_file):
    with open(file, 'w') as sbatch_file:
        ...
        for i in range(1, job_num+1):
            ...
            result_path = "OptiLock_result_" + circuit_name + "_" + str(key_size) + "_" + str(i) + "_h_" + str(h_hop) + "_scope.csv"
        ...
        sbatch_file.write("python ${" + "ROOT_DIR}/OptiLock_SCOPE_Searching.py --key-size ${"  +"KEYSIZE} --result-path ${" + "RESULT_PATH}" +" --target-path ${" + "TARGET_PATH} --iteration ${" + "ITER} --int-temp ${" + "INT_TEMP} --h-hop ${" + "H_HOP} --train-mark ${" + "TRAIN_MARK} --bin-num ${" + "BIN_NUM} --start-num ${" + "START_NUM} --total-num ${" + "TOTAL_NUM} --exp-num ${" + "EXP_NUM} --output-file ${" + "OUTPUT_FILE}\n")
...
if __name__ == '__main__':
    ...
    for circuit_name in circuit_name_list:
        ...
        start_num = 1
        total_num = 5
        iter_num = 1  # need to change later
        init_temp = 800
        bin_num = 1
        output_file = root_dir + "/optilock_" + circuit_name + "_locked.bench"
        train_sbatch_location = "./OptiLock_SCOPE_script"
        ...
        file = train_sbatch_location + "/" + circuit_name + "_optilock_SCOPE.sh"
