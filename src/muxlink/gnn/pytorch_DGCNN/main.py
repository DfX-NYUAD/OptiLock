...
import math
import pdb
from muxlink.gnn.pytorch_DGCNN.DGCNN_embedding import DGCNN
from muxlink.gnn.pytorch_DGCNN.mlp_dropout import MLPClassifier, MLPRegression
from sklearn import metrics
from muxlink.gnn.pytorch_DGCNN.util import cmd_args, load_data
...
class Classifier(nn.Module):
    ...
    def PrepareFeatureLabel(self, batch_graph):
        ...
        # print("the length of batch graph is: ", len(batch_graph))
        for i in range(len(batch_graph)):
            labels[i] = batch_graph[i].label
            n_nodes += batch_graph[i].num_nodes
            if node_tag_flag == True:
                # print("what is the batch graph node tags", batch_graph[i].node_tags)
                concat_tag += batch_graph[i].node_tags
            if node_feat_flag == True:
                tmp = torch.from_numpy(batch_graph[i].node_features).type('torch.FloatTensor')
                #print("This is temp when reading node features")
                #print(tmp)
        ...
        if node_tag_flag == True:
            #One hot encpding is done here. feat_dim is the maximum label
            concat_tag = torch.LongTensor(concat_tag).view(-1, 1)
            i=0
            for e in concat_tag:
                if int(e)<0:
                    print("YES, WE HAVE NEGAIVE LABELS")
                    concat_tag[i]=int(e)*(-1)+cmd_args.max_n_label
                i=i+1

            node_tag = torch.zeros(n_nodes, cmd_args.feat_dim)
            print("feat_dim", str(cmd_args.feat_dim)) ##test
        ...
        if node_feat_flag and node_tag_flag:
            ...
            node_feat_test = torch.cat([node_feat], 1) #test
            node_feat = torch.cat([node_tag.type_as(node_feat), node_feat], 1)
            #######################test
            node_tag_test = torch.cat([node_tag.type_as(node_feat)], 1)
            print("testing for the size of node_feat_flag")
            print("Test for size of node_tag " + str(len(node_tag_test[0])))
            print("Test for size of node_tag " + str(len(node_feat_test[0])))
            # 47+8 = 55; 54+8 = 62
            ###########################
            # check the size of node_tage and node_feat to see if there is some changes.
            print("This is the length of the features "+str(len(node_feat[0])))
