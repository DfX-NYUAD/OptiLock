...
from muxlink.gnn.pytorch_DGCNN.lib.gnn_lib import GNNLIB
from muxlink.gnn.pytorch_DGCNN.lib.pytorch_util import weights_init, gnn_spmm
...
class DGCNN(nn.Module):
    def __init__(self, output_dim, num_node_feats, num_edge_feats, latent_dim=[32, 32, 32, 1], k=30, conv1d_channels=[16, 32], conv1d_kws=[0, 5], conv1d_activation='ReLU'):
        ...
        print("This is the output_dim "+str(output_dim))
        super(DGCNN, self).__init__()
        self.latent_dim = latent_dim
        self.output_dim = output_dim
        self.num_node_feats = num_node_feats
        print("node features are "+ str(num_node_feats))
        self.num_edge_feats = num_edge_feats
        self.k = k
        self.total_latent_dim = sum(latent_dim)
        print("latent dimension "+str(self.total_latent_dim))
        ...
        #if num_edge_feats > 0:
        #    print("yes i am inside "+str(num_edge_feats))
    ...
    def forward(self, graph_list, node_feat, edge_feat):
        print("I am in forward function")
        #print(graph_list)
        ...
        #your_file='/home/nimisha/Desktop/Lilas/python_scripts/SEAL/Python_InterLock_two_circuits/feature.txt'
        #np.savetxt(your_file, node_feat.numpy())
        ...
        #your_file='/home/nimisha/Desktop/Lilas/python_scripts/SEAL/Python_InterLock_two_circuits/subgraph_embedding.txt'
        #print(h[0].size())
        #np.savetxt(your_file, h[0].exp().cpu().detach().numpy())
        return h
    ...
    def sortpooling_embedding(self, node_feat, edge_feat, n2n_sp, e2n_sp, subg_sp, graph_sizes, node_degs):
        ...
        if edge_feat is not None:
            ...
            input_edge_linear=edge_feat.reshape(edge_feat.shape[0],1)
            ...
            print(len(node_feat))
            #print(e2n_sp.shape)
            #e2npool_input=gnnedge_feat
            e2npool_input = gnn_spmm(e2n_sp, input_edge_linear)
            #e2npool_input=input_edge_linear
            #print(node_feat)
            print(node_feat.shape)

            print(e2npool_input.shape)
            node_feat = torch.cat([node_feat, e2npool_input], 1)
            print("Figuring out sizes")
