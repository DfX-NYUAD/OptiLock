...
from muxlink.gnn.pytorch_DGCNN.util import GNNGraph
...
def links2subgraphs(A, train_pos, train_neg, test_pos, test_neg, h=1, node_information=None, no_parallel=False):
    ...
    max_n_label = {'value': 1500} ## here we changed from 0 to 60 to 70 to 100 to 200
    def helper(A, links, g_label):
        ...
        if no_parallel:
            ...
            # max_n_label = {'value': 1500}##added by lilas to overwrite the max value
            # print("labels:", [max(n_labels) for _, n_labels, _ in results]) #debug_Zeng
            end = time.time()
...
def node_label(subgraph):
    ...
    #maximum of labels and set the max limit
