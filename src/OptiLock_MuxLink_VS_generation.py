...
def write_sbatch(file, job_name, memory, walltime, output, job_num, root_dir, circuit_name, h_hop, train_mark, key_size, start_num, total_num, iter_num, init_temp, bin_num, output_file):
    with open(file, 'w') as sbatch_file:
        ...
        for i in range(1, job_num+1):
            ...
            result_path = "OptiLock_result_" + circuit_name + "_" + str(key_size) + "_" + str(i) + "_h_" + str(h_hop) + "_VS.csv"
        ...
        if circuit_name in ["b14", "b15", "b16", "b17", "b18", "b19", "b20"]:
            file_circuit_name = circuit_name + "_C"
        else:
            file_circuit_name = circuit_name
        sbatch_file.write("export TARGET_PATH=\"" + "../data/original/" + circuit_name + "_ori/" + file_circuit_name + ".bench" + "\"\n")
        ...
        sbatch_file.write("python ${" + "ROOT_DIR}/OptiLock_MuxLink_VS_Searching.py --key-size ${"  +"KEYSIZE} --result-path ${" + "RESULT_PATH}" +" --target-path ${" + "TARGET_PATH} --iteration ${" + "ITER} --int-temp ${" + "INT_TEMP} --h-hop ${" + "H_HOP} --train-mark ${" + "TRAIN_MARK} --bin-num ${" + "BIN_NUM} --start-num ${" + "START_NUM} --total-num ${" + "TOTAL_NUM} --exp-num ${" + "EXP_NUM} --output-file ${" + "OUTPUT_FILE}\n")

if __name__ == '__main__':
    circuit_name_list = ['b14']
    for circuit_name in circuit_name_list:
        
        root_dir = os.getcwd()
        job_name = circuit_name + "_VS"
        memory = "70G"
        walltime = "168:00:00"
        output = "OptiLock"
        job_num = 1
        ...
        init_temp = 800
        bin_num = 1000 # 1000 for vertical searching, but it does not any meaning
        output_file = "../optilock_" + circuit_name + "_locked.bench"
        train_sbatch_location = "./OptiLock_VS_script"
        ...
        file = train_sbatch_location + "/" + circuit_name + "_optilock_VS.sh"
