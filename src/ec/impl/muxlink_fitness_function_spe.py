from ec.impl.kgs_ops.muxlink_base_spe import MuxLinkBaseSpe
...
class MuxLinkFitnessFunctionSpe(FitnessFunction):
    ...
    def __init__(self, target_file_path, locked_file_path, h_hop, chosen_sol, train_mark, epochs=100):
        ...
        MuxLinkBaseSpe.instance().load(target_file_path, locked_file_path, h_hop, chosen_sol, train_mark, epochs)
    ...
    def evaluate(self, solutions: [KGSSolution]):
        ...
        for solution in solutions:
            ...
            # get the target name
            target_name = MuxLinkBaseSpe.instance().get_target()
            print("what is my target name2", target_name, flush=True)
            acc, prec, kpa = MuxLinkBaseSpe.instance().attack(solution)
    ...
    def evaluate_exact(self, solutions: [KGSSolution]):
        ...
        for solution in solutions:
            ...
            kpa, uni_list, wrong_list = MuxLinkBaseSpe.instance().attack_exact(solution)
    ...
    def evaluate_thread(self, file_num,solutions: [KGSSolution]):
        ...
        for solution in solutions:
            # evaluate (attack) kgss
            acc, prec, kpa = MuxLinkBaseSpe.instance().attack_thread(file_num, solution)
