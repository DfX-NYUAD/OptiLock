...
class MuxLinkBaseLarge:
    ...
    def load(self, target_file_path, locked_file_path, h_hop, chosen_sol, train_mark, epochs=5):
        ...
        if train_mark == True:
            print("why I am here")
            if lock_mark == False:
                self.muxlink.train(self.netlist_str, lock_mark, key=512, epochs=epochs)
            else:
    ...
    def decode(self, kgss: KGSSolution):
        ...
        return self.muxlink.decode_large_scope(self.netlist_str, kgss)
    ...
    def attack(self, solution: KGSSolution):
        ...
        text_file.close()
        print("what is my target name", self.target, flush=True)
        # return 1,1,0.8
