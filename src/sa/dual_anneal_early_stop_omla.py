        ...
        if step < dim:
            ...
            while need_to_check: 
                if kgss_lock.check_same_gs():# or kgss_lock.check_cycles():
                    ...
                    updated_kgss = kgss_lock.modify_same_gs_unchanged(kgss_edge_dict, found_pairs)
                    if updated_kgss == None:
                        return None
                    # print("same element?")
                    # x_visit = kgss_info.kgss_delock(updated_kgss)
                    # print("check cycles?")
                    cycle_kgss = updated_kgss
                    # cycle_kgss = kgss_lock.check_cycle_pair_unchanged(kgss_edge_dict, found_pairs)
                    if cycle_kgss == None:
                        return None
...
def dual_annealing(func, bounds, args=(), maxiter=1000,
    ...
    early_stopping = EarlyStopping(min_delta=0.005, patience=10000, required_stops=1)
