...
# define the kgss solution class - 
class KGSSInfo:
    """
    ...
    def __init__(self, *args):
        ...
        # self.founded_pairs = args[3] # here the foundpairs is not required
...
class VisitingDistribution:
    ...
    # - simplify the visiting function
    def visit_func_part(self, x, visits, upper_sample, lower_sample):
        ...
        return x_visit
    #  - simplify the visiting function1
    def visit_func_part1(self,step, dim, x, visit, x_visit):
        ...
        return x_visit
    #  - change random seed for the random generator
    ...
    def visiting(self, x, step, temperature, kgss_info):
        ...
        if step < dim:
            ...
            need_to_check = True
            while need_to_check: 
                if kgss_lock.check_same_gs() or kgss_lock.check_cycles():
                    kgss_edge_dict = kgss_info.get_edge_dict()
                    updated_kgss = kgss_lock.modify_same_gs(kgss_edge_dict)
                    # print("same element?")
                    # x_visit = kgss_info.kgss_delock(updated_kgss)
                    # print("check cycles?")
                    cycle_kgss = kgss_lock.check_cycle_pair(kgss_edge_dict)
...
class EnergyState:
    ...
    def generate_sol(self, rand_gen, kgss_info):
        # : generate the x_temp one by one to ignore the same gs
        used_gs = []
        x_all = []
        counter = 0
        lower_temp= self.lower[0:2]
        print("lower_temp in SA ", lower_temp)
        upper_temp = self.upper[0:2]
        print("upper_temp in SA ", upper_temp)
    ...
    def reset(self, func_wrapper, rand_gen, kgss_info, x0=None):
        ...
        if x0 is None:
            ...
            if kgss_lock.check_same_gs() or kgss_lock.check_cycles():
                print("same gs debug in SA ")
                # print("repeat gs in SA ", kgss_lock.check_all_same_gs())
                kgss_edge_dict = kgss_info.get_edge_dict()
                updated_kgss = kgss_lock.modify_same_gs(kgss_edge_dict)
                cycle_kgss = kgss_lock.check_cycle_pair(kgss_edge_dict)
                # updated_kgss = kgss_lock.modify_same_gs_selected(kgss_edge_dict)
                ...
                #                                      size=len(self.lower))
                # # self.current_location = self.generate_sol(rand_gen, kgss_info)
                # # print("new x0 in SA ", self.current_location)
                # kgss_lock = kgss_info.kgss_lock(self.current_location)
        else:
...
class StrategyChain:
    ...
    def run(self, step, temperature, kgss_info):
        ...
        for j in range(self.energy_state.current_location.size * 2):
            ...
            # fitness_function = MuxLinkFitnessFunction(target_path, epochs=100)
                ...
                print("Debug-SA: x_visit:"+str(x_visit))
...
def dual_annealing(func, bounds, args=(), maxiter=1000,
    ...
    #  - initialization of the kgss info
    ...
    # : reset the energy state with kgss solution
    energy_state.reset(func_wrapper, rand_state, kgss_info, x0)
    # : add the function to check the early-stopping condition
    # early_stopping = EarlyStopping(min_delta=0.005, patience=100)
    ...
    while(not need_to_stop):
        for i in range(maxiter):
            ...
            # : add kgss_info as the last parameter
            val = strategy_chain.run(i, temperature, kgss_info)
            if val is not None:
                message.append(val)
                need_to_stop = True
